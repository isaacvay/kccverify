...
export default function Navbar() {
 ...
 // Déclare que user peut être de type User ou null
 const [user, setUser] = useState<User | null>(null);

 useEffect(() => {
   const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
     setUser(currentUser); // currentUser est de type User | null
   });
   return () => unsubscribe();
 }, []);

  // Définir le lien d'authentification selon l'état de l'utilisateur
  const authLink: NavLink = user
    ? { path: "/dashboard", label: "Dashboard", isCta: true }
        ...
        <div className="flex justify-between items-center h-20">
          ...
          <ul className="hidden md:flex items-center gap-8">
            {navLinks.map(({ path, label, isCta }) => (
              <li key={path}>
                <Link href={path} className={`${linkClasses(path, isCta)} ${isCta ? "shadow-md" : ""}`}>
                  {label}
                </Link>
              </li>
            ))}
          ...
          >
            <div className="space-y-2">
              <span
                className={`block w-6 h-0.5 bg-[#01B4AC] transition-transform ${isMenuOpen ? "rotate-45 translate-y-1.5" : ""}`}
              ></span>
            ...
            <div className="h-screen flex flex-col items-center justify-center space-y-6 p-4">
              {navLinks.map(({ path, label, isCta }) => (
                <Link
                  key={path}
                  href={path}
                  className={`text-xl w-full text-center ${linkClasses(path, isCta)} ${
                    isCta ? "!px-8 !py-4 mx-auto max-w-xs" : "py-3"
                  }`}
                  onClick={() => setIsMenuOpen(false)}
                >
                  {label}
